...
#Image.fromarray(img8Bit).save(id + '.bmp')
...
def buildAnimation():
    ...
    for picture in range(1,3):
        for frame in range(1,50):
            try:
                images.append(buildFile())
            except:
                print("Error appending frame " + str(frame) + " from image " + str(picture))
    
    imageio.mimsave("vid.gif", images, duration = 1.5)
